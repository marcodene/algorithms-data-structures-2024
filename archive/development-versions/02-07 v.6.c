...
void aggiungi_lotto_ad_alimento(alimento_nodo* alimento, int quantita, int scadenza){
    ...
    // controllo se è già presente un lotto con la stessa scadenza e in caso aggiorno quel lotto
    int j=0;
    while(j<alimento->lotti.heapsize && alimento->lotti.data[j].scadenza!=scadenza)
        j++;
    if(j<alimento->lotti.heapsize){
        alimento->quantita_disponibile += quantita;
        alimento->lotti.data[j].quantita += quantita;
        return;
    }


    // se non è già presente un lotto con quella scadenza ne aggiungo uno nuovo
...
void aggiungi_ad_ordini_sospesi (ordine_nodo**primo_della_lista, ordine_nodo**ultimo_della_lista, ordine_nodo*ordine_da_inserire){
...
void aggiungi_ad_ordini_pronti ( ordine_nodo*ordine_da_inserire){

    // se non è già presente un lotto con quella scadenza ne aggiungo uno nuovo

    if( ordini.pronti.length==0){
        ordini.pronti.data = realloc(ordini.pronti.data, (ordini.pronti.length+2)*sizeof(ordine_nodo));
        ordini.pronti.length = 2;
    } else if(ordini.pronti.length==ordini.pronti.heapsize){
        // se non ho spazio raddoppio la dimensione dello heap
        ordini.pronti.data = realloc(ordini.pronti.data, (ordini.pronti.length + 5)*sizeof(ordine_nodo));
        ordini.pronti.length += 5;
    } else if(ordini.pronti.heapsize>0 && ordini.pronti.heapsize <= ordini.pronti.length / 4){
        // ridimensione la lunghezza se essa è troppo grande rispetto a heapsize
        ordini.pronti.data = realloc(ordini.pronti.data, (ordini.pronti.length / 2)*sizeof(ordine_nodo));
        ordini.pronti.length /= 2;
    }
    ...
    ordini.pronti.data[ordini.pronti.heapsize] = ordine_da_inserire;

    ordini.pronti.heapsize++;

    // aggiusto lo heap visto che ho aggiunto un elem in fondo
    int i=ordini.pronti.heapsize-1;
    int parent_index = (int)floor((i-1)/2);
    while(i!=0 && ordini.pronti.data[parent_index]->istante_arrivo > ordini.pronti.data[i]->istante_arrivo){
        ordine_nodo * temp = ordini.pronti.data[i];
        ordini.pronti.data[i] = ordini.pronti.data[parent_index];
        ordini.pronti.data[parent_index] = temp;
        i = parent_index;
        parent_index = (int)floor((i-1)/2);
    }
...
void istante_arrivo_heapify(ordini_heap * pronti, int i){
    int left = 2*i+1;     // left = LEFT(i);
    int right = 2*i + 2;     // right = RIGHT(i);
    int new_min;

    if(left < pronti->heapsize &&  pronti->data[left]->istante_arrivo<pronti->data[i]->istante_arrivo )
        new_min = left;
    else new_min = i;

    if(right < pronti->heapsize &&  pronti->data[right]->istante_arrivo<pronti->data[new_min]->istante_arrivo )
        new_min = right;

    if (new_min != i){
        ordine_nodo* temp = pronti->data[i];
        pronti->data[i] = pronti->data[new_min];
        pronti->data[new_min] = temp;
        istante_arrivo_heapify(pronti, new_min);
    }
}

void build_istante_arrivo_heap(ordini_heap * A, int heapsize) {
    for (int i = heapsize / 2 - 1; i >= 0; i--)
        istante_arrivo_heapify(A, i);
}

ordine_nodo * cancella_ordine_tempo_min(ordini_heap * ordini){
    if(ordini->heapsize < 1)
        return NULL;
    ordine_nodo * temp = ordini->data[0];
    ordini->data[0] = ordini->data[ ordini->heapsize-1 ];
    ordini->heapsize -= 1;
    istante_arrivo_heapify(ordini, 0);
    return temp;
}
...
int controlla_disponibilita_dispensa(alimento_nodo* alimento, int quantita_richiesta){
    ...
    if (alimento==NULL || alimento->lotti.data == NULL || alimento->lotti.heapsize == 0) {
        return 0;  
    }
...
void sposta_da_sospesi_a_pronti(ordine_nodo*ordine, ordine_nodo* prec){
    ...
    aggiungi_ad_ordini_pronti(ordine);
...
void cancella_e_stampa_ordini_pre_caricamento(ordine_nodo * x){
    if(x!=NULL){
        cancella_e_stampa_ordini_pre_caricamento(x->left);
        printf("\n%d %s %d", x->istante_arrivo, x->ricetta->nome, x->quantita);
        x->ricetta->num_ordini_presenti--;
        cancella_e_stampa_ordini_pre_caricamento(x->right);
        free(x);
    }
}

void aggiungi_a_ordini_pre_caricamento(ordine_nodo**root, ordine_nodo*z){
    
    ordine_nodo* x = *root;
    ordine_nodo*y= NULL;
    while(x!=NULL){
        y=x;
        if(z->peso > x->peso || ((z->peso == x->peso)&&(z->istante_arrivo < x->istante_arrivo)))
            x = x->left;
        else
            x = x->right;
    }
    ...
    if(y==NULL)
        *root=z;
    else if(z->peso > y->peso || ((z->peso == y->peso)&&(z->istante_arrivo < y->istante_arrivo)))
        y->left = z;
    else
        y->right = z;
}
...
void aggiungi_ricetta(){    // suppongo la stringa istruzione sia gia stata letta
    ...
    if(alreadyExists)
        ...
        while( sscanf(pos_input, "%s %d", nome_ingrediente, &quantita) == 2 ){
...
void rimuovi_ricetta(){
    ...
    if(ricetta==NULL)
        ...
        if( ricetta->num_ordini_presenti !=0 )
            printf("ordini in sospeso");
        else{
...
void ordine(){
    ...
    if(!ricetta_ordinata)
        ...
        if(controlla_ingredienti(ordine)){
            ...
            aggiungi_ad_ordini_sospesi(&(ordini.sospesi), &(ordini.sospesi_coda), ordine);

        ordine->ricetta->num_ordini_presenti ++;
...
void carica_corriere(){
    if(ordini.pronti.heapsize == 0 || ordini.pronti.data[0]->peso > corriere.capienza){
        printf("\ncamioncino vuoto");
        return;
    }

    int sum = ordini.pronti.data[0]->peso;
    ordine_nodo * ordine_da_spostare;
    while(sum < corriere.capienza && ordini.pronti.heapsize !=0){
        ordine_da_spostare = cancella_ordine_tempo_min(&ordini.pronti);
        aggiungi_a_ordini_pre_caricamento(&ordini.pre_caricamento, ordine_da_spostare);
        sum += ordini.pronti.data[0]->peso;
    }

    cancella_e_stampa_ordini_pre_caricamento(ordini.pre_caricamento);
    ordini.pre_caricamento = NULL;
