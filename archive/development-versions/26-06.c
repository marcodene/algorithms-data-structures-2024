...
void rimuovi_ricetta_da_ricettario (ricetta_nodo**root, ricetta_nodo * z){
    ...
    if(y->parent == NULL)
        *root = x;
        ...
        y->parent->right = x;
    if(y!=z) { // copio le informazioni di y all'interno del nodo z
        strcpy(z->nome, y->nome);
        dealloca_ingredienti_da_ricetta(z->ingredienti);
        z->ingredienti = y->ingredienti;
        z->peso = y->peso;
    }else
        dealloca_ingredienti_da_ricetta(z->ingredienti);
...
int conta_ordini_da_caricare(){
    ...
    while (curr!=NULL){
        if(sum + curr->peso <= corriere.capienza){
            sum += curr->peso;
            cnt++;
        }
        curr = curr->next;
...
void aggiungi_ricetta(){    // suppongo la stringa istruzione sia gia stata letta
    ...
    while( scanf("%s", nome_ingrediente) == 1 && !is_istruzione(nome_ingrediente)){
...
void rimuovi_ricetta(){
    ...
    if(!feof(stdin) && scanf("%s", istruzione)!=1)
...
void ordine(){
    ...
    if (scanf("%s %d", nome_ricetta, &quantita_ordine) != 2) {
    ...
    //leggi all'interno di istruzione la prossima istruzione e return
    if(!feof(stdin) && scanf("%s", istruzione)!=1)
        printf("mancata lettura istruzione");
...
void rifornimento(){
    ...
    while (scanf("%s", nome_alimento)==1 && !is_istruzione(nome_alimento)){
        if(scanf("%d %d", &quantita, &scadenza)!=2)
    ...
    // faccio strcpy da alimento, che contiene la nuova istruzione, a istruzione e faccio return
    strcpy(istruzione, nome_alimento);
...
void carica_corriere(){
    ...
    ordine_nodo ** A = (ordine_nodo **)calloc(n, sizeof(ordine_nodo));

    // con n eliminazioni in testa sposto gli ordini da pronti ad A
    ordine_nodo * curr = ordini.pronti;
    ordine_nodo * prec = NULL;
    int sum=0;
    int i=0;
    ...
    // faccio sull'array un heapsort che me lo ordina per peso decrescente, in caso di parita di peso si ordina cronologicamente. 
    heapsort_per_peso(A, n);

    // faccio delle free di ogni elemento dell'array e faccio stampe. Dopo faccio anche la free dell'array stesso
    for(int i=0; i<n; i++){
        printf("\n%d %s %d", A[i]->istante_arrivo, A[i]->ricetta->nome, A[i]->quantita);
        free(A[i]);
    }
    free(A);
...
int main(){
    ...
    if(scanf("%d %d", &(corriere.periodicita), &(corriere.capienza))!=2){
